...
def train_and_save(dynamic, models, epochs, base_train_size, test_size, weights, curricular, setup_name):
    ...
    for model_no in tqdm(models, unit="model_trained", leave=False):
        ...
        #for epoch, train_size in enumerate(tqdm(train_sizes[i:], unit='epoch', leave=False)):
            ...
            # Only for dynUCB
            if epoch==0: 
                trainer.mcts.weights = [15.0 * 0.9**1, 1]
            elif epoch==20: 
                trainer.mcts.weights = [15.0 * 0.9**2, 1]
            elif epoch==40:
                trainer.mcts.weights = [15.0 * 0.9**3, 1]
            elif epoch==60:
                trainer.mcts.weights = [15.0 * 0.9**4, 1]
            elif epoch==80:
                trainer.mcts.weights = [15.0 * 0.9**5, 1]
            elif epoch==150:
                trainer.mcts.weights = [15.0 * 0.9**6, 1]
            elif epoch==120:
                trainer.mcts.weights = [15.0 * 0.9**7, 1]
