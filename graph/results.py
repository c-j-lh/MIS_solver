...
# 'newcinc', 'newtrain100', 'newtrain200', 'newtrain300', 'chardened', 'hardened'
setup_names = ('newcinc', 'newtrain100', 'newtrain200', 'newtrain300', 'chardened', 'hardened')
lay_names = ('Curriculum-Increasing', 'Train100', 'Train200', 'Train300', "Curriculum-Hard-Graphs", "Hard-Graphs")
...
for setup_name in setup_names:
    with open('ctrain_time/{}_0.pickle'.format(setup_name), 'rb') as file:
        ...
        #if setup_name in setup_names[:-2]:
        #times[-1] = times[-1][0::5]
...
plot_epochns = [list(range(0, 5 * raw_log.shape[1], 5)
                     if i < 4
                     else range(raw_log.shape[1]))
                for i, raw_log in enumerate(raw_logs)]

# Plotting average graphs
plt.figure(no_graphs)
for i, (setup_name, setup, raw_log, time, linestyle, lay_name, plot_epochn) in enumerate(zip(setup_names, average, raw_logs, times, linestyles, lay_names, plot_epochns)):
    ...
    plt.plot(plot_epochn, setup, label=lay_name, linestyle=linestyle)




# Plotting results for each graph
max_epoch = max(map(max, plot_epochns))
for setup_name, setup, raw_log, time, linestyle, lay_name, plot_epochn in zip(setup_names, by_graph, raw_logs, times, linestyles, lay_names, plot_epochns):
    for graph_no, graph  in enumerate(setup):
        label = '{}: {} models, {} epochs'.format(setup_name, raw_log.shape[0], raw_log.shape[1])
        plt.figure(graph_no)
        plt.plot(plot_epochn, graph, label=lay_name, linestyle=linestyle)

# Plotting average and labels
for graph_no, graphname, plot_epochn in zip_longest(range(no_graphs + 1), graphnames, plot_epochns):
    plt.figure(graph_no)
    if ideal and graph_no < len(ideal):
        plt.plot([0, max_epoch], 2 * [ideal[graph_no]], label="Actual $\\alpha(G)$")
    ...
    plt.title('Graph of setups\' performance during training against epoch\n{} during training'
