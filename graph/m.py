...
setup_names = ('newm.25', 'm.30', 'm.40', 'newm.5', 'm.60', 'm.70', 'train100', 'm2.0')  #, 'chardened', 'dynUCB') 
lay_names = ('{:3}% iterations'.format(int(100*i)) for i in [0.25, 0.3, 0.4, 0.5, 0.6, 0.7, 1, 2]) #, 'Hard-Graphs')
...
for setup_name in setup_names:
    ...
    print(len(raw_logs[-1]), 'models for', setup_name)
...
plot_epochns = []
for i, (setup_name, setup, raw_log, lay_name, time) in enumerate(zip(setup_names, average, raw_logs, lay_names, times)):
    if setup_name == 'train100':
        fplot(time, setup, label=lay_name)
    else:
        fplot(([0.0] + time)[0::5], setup, label=lay_name)
    plt.gca().set_xlim([None, 5])
